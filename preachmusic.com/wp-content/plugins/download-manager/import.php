...
<b>Select Dir:</b>
    ...
    <div id="path">
    <form method="post" action="admin.php?page=file-manager/importable-files">
...
<td valign="top">    <?php $wpdmimported = explode(",", $_COOKIE['wpdmimported']); ?>
<form action="" method="post">
<table><tr> 
    <td><select data-placeholder="Assign category" id="cats" name="cats" multiple="multiple" style="width: 600px;">    
   <?php wpdm_dropdown_categories(); ?>
  ...
  <table cellspacing="0" class="widefat fixed">
	...
	$k = 0;
    $limit = 50;
    $total = count($fileinfo);
    $p = $_GET['paged']?$_GET['paged']:1;
	$s = ($p-1)*$limit;
    $max = $s+$limit;
    if($max>$total) $max = $total;
	for($index=$s; $index<$max; $index++): $value = $fileinfo[$index]; ?>
      <tr for="file-<?php echo $index; ?>" valign="top" class="importfilelist" id="<?php echo $index; ?>">
        ...
        <td><input size="20" type="text" id="title<?php echo $index; ?>" name="file[<?php echo $index; ?>][title]"></td>
...
<script type="text/javascript">
     ...
     function dimport(id,file){
       ...
       jQuery.post(ajaxurl,{action:'wpdm_dimport',fname:file, title:jQuery('#title'+id).val(),password:jQuery('#password'+id).val(),access:'guest',description:jQuery('#desc'+id).val(),category:jQuery('#cats').val()},function(res){
