...
function wpdm_flat_url($data){
    ...
    $key = $data['url_key']?$data['url_key']:get_wpdm_meta($data[id],'url_key');
    $key = $key?$key:$data['id'];
    ...
    $data['popup_link'] = "<a href='".site_url("/?".get_option('__wpdm_purl_base','download')."={$data[id]}&mode=popup")."'  class='popup-link' >$link_label</a>";
    return $data;
}
