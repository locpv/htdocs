=== Plugin Name ===
Contributors: domnoen.khamfu
Donate link: https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=982D8YWYN8Q2Y&lc=US&item_name=Donation%20for%20Wordpress%20Twitter%20Widget&currency_code=USD&bn=PP%2dDonationsBF%3abtn_donateCC_LG%2egif%3aNonHosted
Tags: twitter, twitter widget, twitter feeds, twitter timeline, twitter api, twitter 1.1, free widget wordpress, free twitter, twitter widget for wordpress
Requires at least: 3.0.1
Tested up to: 3.5.2
Stable tag: 1.1
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

free twitter widget for wordpress with api 1.1.

== Description ==

WP Twitter widget easy to create twitter feed in your blog for settings Twitter API support display user timeline a collection recent Tweets posted by the user indicated by the screen_name or user_id parameters. Each user timeline protected, user will authenticated for use twitter api approved follower of the timeline.

== Installation ==

This section describes how to install the plugin and get it working.

e.g.

1. Upload `plugin wp_twitter_widget_by_ryoking folder` to the `/wp-content/plugins/` directory
2. Activate the plugin through the 'Plugins' menu in WordPress
3. Setting plugin connect with twitter apis at 'Settings menu -> WP Twitter Widget options' Create Twitter application by click this link https://dev.twitter.com/apps/new
4. Consumer key and Consumer secret paste into input box.
5. Click Sign in twitter 
6. Drag and drop widget into sidebar 

== Frequently Asked Questions ==

= Why i need to create twitter app ? =

Twitter api be deprecated when version 1 is closed, now twitter timeline support version 1.1 for use timeline lastest versoin need twitter app for get access tokens.

== Screenshots ==

1. This screen display plugins setting corresponds to screenshot-1.png `/assets/screenshot-1.png` 
2. This is the second screen shot screenshot-2.png `/assets/screenshot-2.png` display widget output at sidebar
Note: that the screenshot is taken from the /assets directory 

== Changelog ==

= 1.0 =
* Main widget addon: addtion twitter connect widget and small options additional changes.
* Add twitter header.

== Upgrade Notice ==

= 1.0 =
Stable version avalable.
= 1.1 =
Add automatic style.

 

== Arbitrary section ==

Returns most recent mentions (tweets containing a users's @screen_name) for the authenticating user. 

== A brief Markdown Example ==

Ordered list:

1. Custom screen name
2. Twitter header
3. Support instagram, youtube, link, checkin, foursqure, etc
4. Support follow

Unordered list:

* Add option footer 
* support process urls 
* hide retwitted and replay user_mentions
* more media support

Here's a example link to [Example](http://australiapods.com/ "Example twitter widget")  
