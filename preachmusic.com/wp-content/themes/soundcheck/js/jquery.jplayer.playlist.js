...
jQuery( document ).ready( function( $ ) {
	 ...
	 * wp_localize_script() - functions.php
	 *
	 */
	var get_template_directory_uri = php_params.get_template_directory_uri,
	    defaultPlaylistJson = php_params.default_playlist.replace( /&quot;/g, '"' ),
	    defaultPlaylist = $.parseJSON( defaultPlaylistJson ),
	    is_home = php_params.is_home,
	    autoplay = php_params.autoplay;
	    
	
	/**
	 * Determine if the Auto Play option is set.
	 * If it is, determine if it's the home page.
	 * If it is the home page, autoplay the album,
	 * if not, then set enable_autopaly to false.
	 * We don't want the album autoplaying on each
	 * page load, just on the home page.
	 *
	 */
	...
	/**		
	 * Creat jPlayer Playlist Object
	 *
	 */
	var myPlaylist = new jPlayerPlaylist({
		jPlayer: '#jquery_jplayer_N',
		cssSelectorAncestor: '#jp_container_N'
	},	// Insert Parsed PHP JSON Playlist
		defaultPlaylist, 
	{
		playlistOptions: {
			enableRemoveControls: false,
			autoPlay: enable_autoplay // coditions for this arguement are above
		},
  		swfPath: get_template_directory_uri + '/js',
		supplied: 'mp3',
		wmode: 'window',
    	ready: function() {
   			//console.log('jPlayer Ready Event Fired');
			hideNotification(); // performs updateCurrentTrackInfo();
    		addTrackNumber();
    	},
    	play: function() {
   			//console.log('jPlayer Play Event Fired');
    		updateCurrentTrackInfo();
    	},
    	ended: function() {
   			//console.log('jPlayer Ended Event Fired');
    		updateCurrentTrackInfo();
		}
	});
    ...
    // jPlayer Controls - Reveal/Hide
    $( '.jp-controls-wrap').hide();
    $( '.ti-media-player').hover( function() {
		$( '.jp-controls-wrap' ).fadeTo( "fast", 1 );
    }, function() {
		$( '.jp-controls-wrap' ).fadeTo( "fast", 0 );
    });
    	
	
	// Update current track information
	function updateCurrentTrackInfo() {
  		var currentItem = myPlaylist.playlist[myPlaylist.current],
            currentTrack = (currentItem.title) ? currentItem.title : 'Title Unknown',
  	        currentArtist = (currentItem.artist) ? 'by ' + currentItem.artist : '',
  	        currentAlbum = (currentItem.album) ? currentItem.album : 'Album Unknown',
  	        currentPoster = (currentItem.poster) ? currentItem.poster : get_template_directory_uri + '/images/default-album-artwork.png';
  	
		$( '.jp-current-track' ).html(currentTrack);
		$( '.jp-current-artist' ).html(currentArtist);
		$( '.jp-current-album' ).html(currentAlbum);
		$( '.jp-current-poster img' ).removeClass( 'preloading' ).attr( 'src', currentPoster ).addClass( 'loaded' );
		//alert(currentAlbum + ' ' + currentArtist + ' ' + currentTrack);
	}
	
	
	// Hide notification messages - Loading & Updated Info
	function hideNotification() {
		$( '.jp-notification' ).fadeTo( 'slow', 0, function() {
			// Update track information once notification has faded out.
			updateCurrentTrackInfo();
			// Remove preloading class and add loaded class
			$( '.jp-type-playlist' ).removeClass( 'preloading' ).addClass( 'loaded' );
		}).hide();
	}
	...
	function addTrackNumber() {
		$( '.jp-track-number' ).remove();
		
		$( 'ul.tracks li' ).each( function() {
			var trackNumber = $( this ).index() + 1;
			$( this ).prepend( '<span class="jp-track-number">' + trackNumber + '.</span>' );
		})
	}
