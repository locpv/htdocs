...
add_action( "admin_print_styles-$of_page", 'ti_optionsframework_styles' );
...
if ( ! function_exists( 'optionsframework_options' ) ) {
	...
	function optionsframework_options() { 
		...
		// Pull all the pages into an array
		$options_albums = array();  
		$options_albums_obj = get_posts( 'post_type=discography&sort_column=post_parent,menu_order' );
		$options_albums[''] = __( 'Select an album:', 'theme-it' );
		foreach ( $options_albums_obj as $album ) 
		{
			$options_albums[$album->ID] = $album->post_title;
		}
		
		// The Options
		$options = array();

		include 'options/options-getting-started.php';
		include 'options/options-content.php';
		include 'options/options-customization.php';
