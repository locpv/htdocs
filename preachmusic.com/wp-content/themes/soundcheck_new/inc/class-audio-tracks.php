...
class Soundcheck_Audio {
	...
	function get_external_url( $attachment_id = null ) {
		...
		return get_post_meta( $attachment_id, 'soundcheck_external_url', true );
	...
	function is_attachement_playable( $attachment_id = null ) {
		...
		$attachment_id = ( null === $attachment_id ) ? get_the_ID() : $attachment_id;
		return get_post_meta( $attachment_id, 'soundcheck_playable', true );
	...
	function track( $attachment = false ) {
		...
		return array( 
			...
			'title'  => strip_tags( $this->get_title( $attachment ) ),
			'artist' => strip_tags( $this->get_artist( $attachment ) ),
			'album'  => strip_tags( $this->get_album( $attachment ) ),
